import { META_POSTFIX, RESUME_HEADER, RSC_POSTFIX, STALE_TIME_HEADER } from "../utils/constants.js";
import { normalizeHtmlFilePath, normalizeMetaFilePath, normalizeRoutePath } from "../utils/path.js";
...
async function tryPprStitch(request: Request, env: EyeballEnv): Promise<Response | null> {
  ...
  // Kick off resume in parallel with shell streaming.
  ...
  const staleTime = shellResponse.headers.get(STALE_TIME_HEADER);
  if (staleTime) {
    headers.set(STALE_TIME_HEADER, staleTime);
  }
...
function shouldConsiderPpr(pathname: string) {
  ...
  if (
    ...
    !lastSegment.endsWith(META_POSTFIX)
  ) {
    return false;
  }
